...
- ノード主導モデル同期: ノードはルーターの `/v1/models` と `/api/models/blob/:model_name` を参照して
...
└──────────────┬───────────────┬────────┘
        ...
        └────────────┘   └────────────┘
               ▲               ▲
               │ /api/nodes, /api/health (registration/heartbeat)
               └───────────────┘
```
...
- ノードは `/v1/models` を参照してモデル一覧を取得します。
  ...
  - 参照できない場合は `/api/models/blob/:model_name` からダウンロードしてローカルに保存します。
...
- POST `/api/health`（ノードからのヘルス/メトリクス送信、`X-Node-Token` 必須）
...
- GET `/api/models/convert`
- GET `/api/models/convert/:task_id`
- DELETE `/api/models/convert/:task_id`
