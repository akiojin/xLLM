...
5. Phase 1 を実行 → contracts, data-model.md, quickstart.md,
   ノード固有ファイル (例: Claude Code用`CLAUDE.md`、
   GitHub Copilot用`.github/copilot-instructions.md`、Gemini CLI用`GEMINI.md`)
6. 憲章チェックセクションを再評価
...
Phase 0 research前に合格必須。Phase 1 design後に再チェック。
...
5. **ノードファイルを漸進的に更新** (O(1)操作):
   - AIアシスタント用に `/scripts/update-assistant-context.sh [claude|gemini|copilot]`
     を実行
...
このセクションは /tasks コマンドが実行することを記述し、/plan 中は実行しない。
