...
| 環境変数 | 説明 | デフォルト値 |
|---------|------|------------|
| `LLMLB_URL` | ルーターURL | `http://127.0.0.1:32768` |
...
- ルーター環境変数名は`LLMLB_*`プレフィックスで統一する
...
- ノードバイナリがインストールされている
- データベースファイル（`~/.llmlb/router.db`）への読み書き権限がある
...
6. ルーター環境変数が`LLMLB_*`プレフィックスで統一されている
...
- 環境変数プレフィックス: Router=LLMLB_*, xLLM=XLLM_*
