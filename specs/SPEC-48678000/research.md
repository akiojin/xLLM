...
### ダウンロード後の整合性検証（SHA-256）

- `supported_models.json` に期待 SHA-256 を記載し、ダウンロード完了後に計算して比較する
- HuggingFace の LFS オブジェクトでは ETag が SHA-256 の場合があるため、取得できる場合は
  ETag とローカル計算値を比較する（HF_TOKEN 認証時も同様）
- 不一致の場合は保存を破棄し、エラーを返す（ログとエラーコードを記録）
- 失敗時はリトライ可能にし、手動対処フロー（再取得/キャッシュ削除）を明記する
...
2. ロックが存在する場合
   - PID と開始時刻を読み取る
   - PID が存在しない/開始時刻がしきい値超過なら stale と判断して削除
   - stale でなければロック解除を待機（タイムアウトは設定可能、既定 5分）
3. ロックが存在しない場合
   - PID + 開始時刻を含むロックファイルをアトミックに作成
   - ダウンロード実行
   - 完了後ロック解除
```
...
1. `.lock` の PID が生存しているか確認
2. PID が存在しなければ stale と判断
3. ログを確認し、問題がないことを確認した上で `.lock` を削除

stale 判定のしきい値（例: 5分）は設定可能とし、削除時はログに記録する。
