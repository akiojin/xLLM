...
- **FR-007**: システムは[要明確化: 対応する量子化方式の一覧が必要]をサポートする必要がある
- **FR-008**: システムは[要明確化: 量子化指定の入口（CLI/API/設定ファイル）の優先順位]に従って量子化を決定する必要がある
- **FR-009**: システムは[要明確化: 量子化指定時のフォールバック有無/条件]に従って動作する必要がある
