...
### ユーザーストーリー4 - xLLMランタイム側でのGPU互換モデル報告 (優先度: P1)
...
- **FR-009**: ~~ノードはsupported_models.jsonをビルド時に埋め込み、GPU互換性判定に使用する~~
  **廃止**: ノードはHuggingFace config.jsonからアーキテクチャを自動検出し、GPU互換性を判定する
...
- **GpuBackend**: ノードのGPU種別（Metal, CUDA, DirectML, ROCm, CPU）※xLLMランタイム側のみ
...
- ロードバランサーはGPUバックエンド情報を保持しない（xLLMランタイム側のみ）
