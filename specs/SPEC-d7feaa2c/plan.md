...
### Metal/CUDA（GPU実行）
- NodeはGPU必須（Apple Silicon/Metal、Windows/CUDAを主対象）
...
## Session 2025-12-31 追加設計 Part 6

### VRAM部分ロード障害設計

**障害時の挙動**:

- ロード途中でVRAM不足が発生した場合、ロード済みテンソルを即座に全解放
- 部分ロード状態を維持しない（クリーンな状態に戻す）
- エラーコード OOM_VRAM を返却

**実装方針**:

```
fn load_model(path) -> Result<Model, Error> {
    let mut loaded_tensors = Vec::new();
    for tensor in tensors {
        match gpu.allocate(tensor.size) {
            Ok(mem) => loaded_tensors.push(mem),
            Err(OOM) => {
                // 全解放してエラー返却
                for t in loaded_tensors { t.free(); }
                return Err(OOM_VRAM);
            }
        }
    }
...
- 予測可能性: 同じリクエストは常に同じ量子化を使用
- デバッグ容易性: 意図しない量子化切り替えによる問題を防止
- コスト予測: VRAM使用量が安定
...
- プラグインクラッシュ検出後、即座に503 Service Unavailable返却
- 再起動完了待ちのキュー待機は行わない
- クライアント側でのリトライを前提とする
...
1. プラグインクラッシュ検出
2. 処理中リクエストに503返却
3. バックグラウンドでプラグイン再起動開始
4. 新規リクエストも再起動完了まで503
5. 再起動完了後、新規リクエスト受付再開

### ストリーミングハング検出設計

**トークン間タイムアウト**:

- 最終トークン出力から5秒間新トークンが出力されない場合をハングと判定
- 30秒の全体タイムアウトとは別の監視機構

**実装方針**:

```
let last_token_time = now();
while generating {
    if now() - last_token_time > 5s {
        // ハング検出 → 強制終了
...
- ストリーミング中の応答停滞を早期検出
- GPUデッドロックや無限ループの迅速な検出
- ユーザー体験向上（30秒待たずに5秒で検出）

### プラグインログ統合設計

**ログ形式**:

- プラグインのstdout/stderrをホストがキャプチャ
- 各行にプラグインIDをプレフィックスとして付与
- タイムスタンプとログレベルも付与

**フォーマット例**:

```
2025-12-31T12:00:00.000Z [llama_cpp] INFO: Loading model...
2025-12-31T12:00:01.000Z [llama_cpp] DEBUG: Layer 1/32 loaded
2025-12-31T12:00:02.000Z [nemotron] WARN: High VRAM usage
```

**実装**:

- プラグインはprintf/fprintf(stderr, ...)で通常通り出力
- ホストがパイプ経由でキャプチャ
- プラグインIDを付与してノードのログストリームに統合

### ロード進捗API設計

**方針: 非公開**

- モデルロード中の進捗率（%）は外部APIに公開しない
- ロード開始/完了/失敗のみを通知
- 内部メトリクスとしては追跡可能（デバッグ用）

**理由**:

- 実装シンプル化を優先
- プラグインごとに進捗計算方法が異なる可能性
- 進捗表示のUI/UXは将来拡張で検討
...
- Completion/Embeddingリクエストに優先度差なし
- 全リクエストを到着順（FIFO）で処理
- QoS機能は本仕様スコープ外
