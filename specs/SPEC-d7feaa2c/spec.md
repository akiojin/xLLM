...
TextManager owns text/embedding runtimes (llama.cpp, safetensors.cpp).
AudioManager and ImageManager wrap whisper.cpp and stable-diffusion.cpp.
Responses API is the recommended interface; Chat Completions remains for
compatibility.
...
- ModelDescriptor.runtime selects the runtime
  (e.g., "llama_cpp", "safetensors_cpp").
...
- Remove plugin configuration (e.g., engine plugins directory and restart
  policy).
...
- Verification can be satisfied via model verification suite or
  integration/E2E coverage, but it must be recorded.
- Model downloads for tests must use `xllm pull` (server mode or `--direct`).
