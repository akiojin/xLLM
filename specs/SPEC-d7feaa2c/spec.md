...
**ルーター（`router/` - Rust製）**: リクエストの受付・振り分け・管理を担当

- OpenAI互換APIの提供（`/v1/chat/completions`, `/v1/embeddings` 等）
- 認証（JWT/APIキー）
- 複数ノードの登録・ヘルスチェック・負荷分散
- モデルカタログ管理（HuggingFace連携）
- ダッシュボードUI提供

**ノード（`node/` - C++製、llama.cppベース）**: 実際のLLM推論を担当

- モデルロード・VRAM管理
- 推論実行（テキスト生成、埋め込み計算）
- GPU管理・ストリーミング
- エンジンプラグインによる複数エンジン対応（本仕様のスコープ）

### ノードの動作モード

**ノード単体モード（スタンドアロン）**:

- Ollamaサーバーと同様のスタンドアロン動作が可能
...
本仕様（SPEC-d7feaa2c）は、ノード側のエンジンローダー抽象化に焦点を当てる。
...
- **DLL?**: Windows???? `gptoss_directml.dll` / `nemotron_directml.dll` ?????????????????? `ALLM_GPTOSS_DML_LIB` / `ALLM_NEMOTRON_DML_LIB` ???DLL?????
