...
- **悪意プラグイン防御**: 防御なし（自己責任）
  - サードパーティプラグインの悪意あるコード（無限ループ、メモリ爆食、ファイル破壊等）に対する防御機構は提供しない
  - プラグインのインストールはユーザーの自己責任
  - 信頼できるソースからのみインストールすることを推奨

**プロファイリング・デバッグ**:

- **パフォーマンスプロファイリング**: 提供しない
  - プラグイン内部のボトルネック特定ツールは提供しない
  - プラグイン開発者が独自に対応

**テンソル並列（分散ロード）**:

- **複数GPU分散**: 将来対応
  - 同一モデルを複数GPUに分散ロード（テンソル並列）は現時点でスコープ外
  - 大規模モデル対応として将来的に検討

**設定ファイル互換性**:

- **バージョンアップ時**: 自動マイグレーション
  - プラグインの設定ファイルは旧バージョンから新バージョンへ自動変換
  - 手動の設定移行は不要

**プラグイン対応形式検証**:

- **形式対応の検証**: manifestを信頼
  - プラグインが「特定形式のみ対応」と主張した場合、manifestの宣言を信頼
  - 実際にロードしてみて失敗した場合はエラーとして処理
  - 事前の動作検証は行わない

**動的コンテキスト長**:

- **推論中の変更**: 許可（次リクエストから）
  - 現在のリクエスト処理中にコンテキスト長を変更した場合、現在のリクエスト完了後に反映
  - 即時反映はしない（処理中のKVキャッシュを破棄しない）

**トークン数事前申告**:

- **生成トークン数申告**: 不要
  - プラグインは生成開始前に予想トークン数を申告する必要なし
  - ホスト側でリソースを動的に調整

**プラグイン競合解決**:

- **複数プラグイン競合時**: 最新バージョン優先
  - 同じモデルアーキテクチャをサポートする複数プラグインが存在する場合、バージョン番号が最も新しいものを選択
  - ベンチマーク結果ではなくバージョンで決定
