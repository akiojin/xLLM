...
**重要**: テスト用モデルの取得は `xllm pull` を使用すること（serverモードまたは `--direct`）。
**重要**: 各テスト後は `xllm rm <MODEL>` でモデルを削除すること（ストレージ節約）。

### 参照モデル（テスト用の正規ID）

必須テストで使用する具体的なモデルIDは `specs/reference-models.md` に記載する。
...
開発者として、gpt-oss-20b/nemotron以外の新しいモデルアーキテクチャを in-process 実装として追加できるようにしたい。

**この優先度の理由**: 将来的な拡張性を確保するため。Llama系、Mistral、Qwen等への対応を可能にする。

**独立テスト**: 新しいアーキテクチャ実装を追加し、対応モデルで推論できることを確認。

**受け入れシナリオ**:

1. **前提** 新しいアーキテクチャ実装を追加、**実行** EngineRegistry に登録、**結果** 対応モデルがロード可能になる
...
- **FR-009**: アーキテクチャを in-process 登録で拡張できる
...
- **Architecture**: モデルアーキテクチャの定義。マネージャ方式で拡張可能
...
- SPEC-d7feaa2c（エンジン統合）- Runtime統合時にマネージャ方式に準拠
