...
class ImageIntegrationFixture : public ::testing::Test {
protected:
    void SetUp() override {
        xllm::set_ready(true);
        temp_dir_ = uniqueTempDir("xllm-image-integration-");
        ...
        server_ = std::make_unique<HttpServer>(18098, *openai_, *node_);
...
TEST_F(ImageIntegrationFixture, ImageGenerationStoresFileAndServesUrl) {
    ...
    std::string filename = path.substr(std::string("/images/").size());
    std::filesystem::path stored_path = std::filesystem::path(temp_dir_) / filename;
